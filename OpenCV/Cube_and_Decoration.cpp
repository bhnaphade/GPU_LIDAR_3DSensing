...
void draw_cube(Mat image)
{
	...
	patch_pts[0][0].x = cb1_2d.x;
	patch_pts[0][0].y = cb1_2d.y;

	patch_pts[0][1].x = cb5_2d.x;
	patch_pts[0][1].y = cb5_2d.y;

	patch_pts[0][2].x = cb6_2d.x;
	patch_pts[0][2].y = cb6_2d.y;

	patch_pts[0][3].x = cb4_2d.x;
	patch_pts[0][3].y = cb4_2d.y;
...
void surface_decoration(Mat image)
{
	...
	else
	{
		...
		Mat d_1 = (cv::Mat_<double>(4, 1) << cube_size, offset, cube_size - offset, 1);
		Mat d_2 = (cv::Mat_<double>(4, 1) << cube_size, offset, offset, 1);
		Mat d_3 = (cv::Mat_<double>(4, 1) << cube_size, cube_size - offset, offset, 1);
		Mat d_4 = (cv::Mat_<double>(4, 1) << cube_size, cube_size - offset, cube_size - offset, 1);
