...
void cudaImageProjection(Mat src,Mat dest)
{
	...
	{ 
     old_src=src;
     cudaImageProjectioncaller(old_src, dest,transMat, xOffset,yOffset,screenCoordinates,
                              (const int)1/*Projection Plane--define Enum*/,
                              (const int) projPlaneDist);
							  imshow("3D World Image", dest);
	old_src=dest;	
	i++;
	}
	if(i==2)
	...
	old_src=dest;
	cudaImageProjectioncaller(old_src, dest,transMat, xOffset,yOffset,screenCoordinates,
                              (const int)2/*Projection Plane--define Enum*/,
                              (const int) projPlaneDist);
							  imshow("3D World Image", dest);
	old_src=dest;						  
		i++;
	...
	old_src=dest;
	cudaImageProjectioncaller(old_src, dest,transMat, xOffset,yOffset,screenCoordinates,
                              (const int)3/*Projection Plane--define Enum*/,
                              (const int) projPlaneDist);
							  imshow("3D World Image", dest);
	old_src=dest;	
	}
