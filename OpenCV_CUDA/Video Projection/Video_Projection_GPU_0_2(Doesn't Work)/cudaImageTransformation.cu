...
static double *d_transformationMat = NULL;
static twoDCoordinates *d_transformedCoordinates = NULL;

__device__ twoDCoordinates
cudaTransform(double* transformationMat,int xValue, int yValue,
              const int planeDistance,const twoDCoordinates offset,
              const int projectionPlane)
{
    twoDCoordinates twoDPoint;
    double matrixResult[transMatRows];
    
    double threeDPoint[transMatRows];
    
    switch(projectionPlane)
    {
        case 1:
            threeDPoint[0] = (double)planeDistance;
            threeDPoint[1] = (double)xValue;
            threeDPoint[2] = (double)yValue;
            break;
        case 2:
            threeDPoint[0] = (double)xValue;
            threeDPoint[1] = (double)planeDistance;
            threeDPoint[2] = (double)yValue;
            break;
        case 3:
            threeDPoint[0] = (double)xValue;
            threeDPoint[1] = (double)yValue;
            threeDPoint[2] = (double)planeDistance;
            break;
        default:
    ...
    twoDPoint.x = (int)((double)planeDistance * matrixResult[0]/matrixResult[2]);

    twoDPoint.x += offset.x;
                                  
    twoDPoint.y = (int)((double)planeDistance * matrixResult[1]/matrixResult[2]);
                                 ...
                                 const int imageHeight)
{
    ...
    unsigned long int transformationMatSize = transMatCols*transMatRows*sizeof(double);
