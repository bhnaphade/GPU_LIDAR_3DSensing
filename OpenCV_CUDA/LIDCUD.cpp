...
Mat x_ax = (cv::Mat_<int>(4, 1) << 400, 0, 0, 1);

Mat y_ax = (cv::Mat_<int>(4, 1) << 0, 400, 0, 1);

Mat z_ax = (cv::Mat_<int>(4, 1) << 0, 0, 400, 1);

//Cube points

Mat cb_1 = (cv::Mat_<int>(4, 1) << cube_size, 0, cube_size, 1);

Mat cb_2 = (cv::Mat_<int>(4, 1) << 0, 0, cube_size, 1);

Mat cb_3 = (cv::Mat_<int>(4, 1) << 0, cube_size, cube_size, 1);

Mat cb_4 = (cv::Mat_<int>(4, 1) << cube_size, cube_size, cube_size, 1);

Mat cb_5 = (cv::Mat_<int>(4, 1) << cube_size, 0, 0, 1);

Mat cb_6 = (cv::Mat_<int>(4, 1) << cube_size, cube_size, 0, 1);

Mat cb_7 = (cv::Mat_<int>(4, 1) << 0, cube_size, 0, 1);
...
Mat sc_1 = (cv::Mat_<int>(4, 1) << 0, 0, sc_size/2, 1);

Mat sc_2 = (cv::Mat_<int>(4, 1) << sc_size, 0, sc_size/2, 1);

Mat sc_3 = (cv::Mat_<int>(4, 1) << sc_size, sc_size, sc_size/2, 1);

Mat sc_4 = (cv::Mat_<int>(4, 1) << 0, sc_size, sc_size/2, 1);


Mat cam_eye = (cv::Mat_<int>(3, 1) << 250, 150, 450);

Mat org = (cv::Mat_<int>(4, 1) << 0, 0, 0, 1);


int rho = sqrt((cam_eye.at<int>(0, 0)*cam_eye.at<int>(0, 0)) + (cam_eye.at<int>(1, 0)*cam_eye.at<int>(1, 0)) + (cam_eye.at<int>(2, 0)*cam_eye.at<int>(2, 0)));

int thetha = (acos(cam_eye.at<int>(0, 0) / sqrt((cam_eye.at<int>(0, 0) * cam_eye.at<int>(0, 0)) + (cam_eye.at<int>(1, 0) * cam_eye.at<int>(1, 0)))));

int phi = (acos(cam_eye.at<int>(2, 0) / rho));
...
Mat trans_mat = (cv::Mat_<int>(4, 4) << -sin(thetha), cos(thetha), 0.0, 0.0,
...
cv::Point2d transform(Mat threed_mat)
{
	...
	twod_pt.x = proj_plane_dist * cam_view_3d.at<int>(0, 0) / cam_view_3d.at<int>(2, 0);
	twod_pt.x += x_off;
	twod_pt.y = proj_plane_dist * cam_view_3d.at<int>(1, 0) / cam_view_3d.at<int>(2, 0);
	twod_pt.y += y_off;
...
void surface_decoration(Mat image)
{
	...
	{
		int offset = (cube_size - k) / 2;

		//Color patch decoration points
		Mat d_1 = (cv::Mat_<int>(4, 1) << sc_size, offset, sc_size - 30 - offset, 1);
		Mat d_2 = (cv::Mat_<int>(4, 1) << sc_size, offset, offset, 1);
		Mat d_3 = (cv::Mat_<int>(4, 1) << sc_size, sc_size - 20 - offset, offset, 1);
		Mat d_4 = (cv::Mat_<int>(4, 1) << sc_size, sc_size - 20 - offset, sc_size - 30 - offset, 1);
...
{	
	...
	for(int i = 0; i<count; i++)
	{
		...
		Mat d_1 = (cv::Mat_<int>(4, 1) << target_pt.x-1, target_pt.y-1, sc_size/2, 1);
		Mat d_2 = (cv::Mat_<int>(4, 1) << target_pt.x-1, target_pt.y+1, sc_size/2, 1);
		Mat d_3 = (cv::Mat_<int>(4, 1) << target_pt.x+1, target_pt.y-1, sc_size/2, 1);
		Mat d_4 = (cv::Mat_<int>(4, 1) << target_pt.x+1, target_pt.y+1, sc_size/2, 1);
